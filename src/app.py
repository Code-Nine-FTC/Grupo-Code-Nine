...
def login():
    if request.method == 'POST':
        ...
        if user: #caso a variável user seja verdadeira
            cursor.execute("SELECT username FROM usuario where email = %s", (email))
...
def create_post():
    if 'user_email' in session:
        if request.method == 'POST':
            ...
            cursor = conn.cursor()
            try:
                cursor.execute("INSERT INTO posts (user_email, username, content) VALUES (%s, %s %s)",
...
def add_comment(post_id):
    if 'user_email' in session:
        if request.method == 'POST':
            ...
            try:
                cursor.execute("INSERT INTO comentarios (post_id, user_email, user_name, content) VALUES (%s, %s, %s, %s)",
                               (post_id, user_email, user_name, content))
                conn.commit()
...
def criar_pergunta():
    if 'user_email' in session:
        if request.method == 'POST':
            ...
            cursor = conn.cursor()
            try:
                cursor.execute("INSERT INTO perguntas (user_email, user_name, texto) VALUES (%s, %s, %s)",
                               (user_email, user_name, texto))
                conn.commit()
    ...
    if 'user_email' in session:
        if request.method == 'POST':
            ...
            if len(texto) > 300:
                return "O comentário excede o tamanho máximo de 300 caracteres."
            try:
                cursor.execute("INSERT INTO respostas_perguntas (perg_id, user_email, user_name, texto) VALUES (%s, %s, %s, %s)",
                               (perg_id, user_email, user_name, texto))
                conn.commit()
